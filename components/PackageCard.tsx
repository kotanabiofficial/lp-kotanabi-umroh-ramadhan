...
const PackageCard: React.FC<PackageCardProps> = ({ name, duration, price, features, isPopular, image }) => {
  return (
    <div className={`bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 ${isPopular ? 'transform md:-translate-y-4 border-4 border-primary shadow-2xl' : 'hover:transform hover:-translate-y-2'}`}>
      ...
      <img src={image} alt={`Paket Umroh ${name}`} className="w-full h-72 object-cover" />
      <div className="p-6">
        ...
        <a href="#kontak" className="w-full block text-center bg-secondary hover:bg-gray-800 text-white font-bold py-3 px-6 rounded-lg transition duration-300">
